#!/usr/bin/env python3
"""Validate exported bootstrap-service readiness assurance evidence.

This repository is not an IPAM, DHCP/metadata, DNS, identity, time, artifact, telemetry,
or platform bootstrap authority. Records contain opaque references to externally owned
lifecycle and service evidence. The checker never allocates addresses, writes DNS,
changes metadata/DHCP, issues credentials, opens service paths, or applies infrastructure.
"""
...
STATUS='EXPORTED_BOOTSTRAP_SERVICE_EVIDENCE_NOT_BOOTSTRAP_AUTHORITY'
STATES={'CURRENT_QUALIFIED','REVIEW_DUE','DEPENDENCY_DUE','TRANSITION_DUE','GAPS_OPEN','UNCERTAIN'}
...
RECORD_KEYS={
    ...
    'authoritative_lifecycle','minimum_dependencies','management_execution',
    'failure_behavior','steady_state_transition','residual_gaps','source_refs'
}
SCOPE_KEYS={
    'site_ref','service_class_ref','platform_profile_ref','bootstrap_profile_ref',
    'address_family_profile_ref','owner_ref','accepted_at','review_by'
}
LIFECYCLE_KEYS={
    'reservation_ref','ipam_allocation_ref','dns_registration_ref',
    'address_assignment_method_ref','dhcp_metadata_profile_ref',
    'address_name_reconciliation_ref','observed_at','valid_until'
}
DEPENDENCY_KEYS={
    'resolver_profile_ref','time_profile_ref','identity_trust_ref','certificate_trust_ref',
    'key_access_ref','artifact_repository_ref','telemetry_profile_ref',
    'service_reply_assurance_ref','observed_at','valid_until'
}
MANAGEMENT_KEYS={
    'restricted_console_oob_ref','initial_privileged_trust_ref',
    'restricted_execution_environment_ref','artifact_integrity_ref',
    'configuration_state_recovery_ref','management_isolation_ref',
    'dependency_cut_review_ref','observed_at','valid_until'
}
FAILURE_KEYS={
    'resolver_loss_ref','time_loss_ref','repository_loss_ref','telemetry_loss_ref',
    'identity_trust_loss_ref','no_unrestricted_fallback_ref',
    'approved_recovery_path_ref','observed_at','valid_until'
}
TRANSITION_KEYS={
    'temporary_dependency_inventory_ref','steady_state_mapping_ref',
    'temporary_credential_revocation_ref','temporary_route_grant_cleanup_ref',
    'recovery_material_preservation_ref','handover_acceptance_ref',
...
def load(path:Path=INDEX):
    ...
    if len(raw)>1024*1024:
        raise ValueError('Bootstrap assurance index exceeds bounded size')
    def pairs(items):
        out={}
        for k,v in items:
            if k in out:
    ...
    if not isinstance(value,dict):
        raise ValueError('Bootstrap assurance index must be an object')
    return value
...
def validate_record(record,as_of,root=ROOT):
    if not isinstance(record,dict) or set(record)!=RECORD_KEYS:
        raise ValueError('Bootstrap assurance record has unexpected or missing fields')
    ...
    if record['state'] not in STATES:
        raise ValueError('Unknown bootstrap assurance state')

    scope=record['scope']
    if not isinstance(scope,dict) or set(scope)!=SCOPE_KEYS:
        raise ValueError('Bootstrap scope shape invalid')
    for key in ('site_ref','service_class_ref','platform_profile_ref','bootstrap_profile_ref','address_family_profile_ref','owner_ref'):
    ...
    _,lifecycle_valid=evidence_block(record['authoritative_lifecycle'],LIFECYCLE_KEYS,'authoritative_lifecycle',as_of)
    _,dependencies_valid=evidence_block(record['minimum_dependencies'],DEPENDENCY_KEYS,'minimum_dependencies',as_of)
    _,management_valid=evidence_block(record['management_execution'],MANAGEMENT_KEYS,'management_execution',as_of)
    _,failure_valid=evidence_block(record['failure_behavior'],FAILURE_KEYS,'failure_behavior',as_of)
    _,transition_valid=evidence_block(record['steady_state_transition'],TRANSITION_KEYS,'steady_state_transition',as_of)
    ...
    dependency_due=as_of>=min(lifecycle_valid,dependencies_valid)
    transition_due=as_of>=min(management_valid,failure_valid,transition_valid)

    state=record['state']
    if state=='CURRENT_QUALIFIED':
        if review_due or dependency_due or transition_due or open_gaps:
            raise ValueError('CURRENT_QUALIFIED bootstrap evidence is stale or incomplete')
        ...
        if review_due or not dependency_due:
            raise ValueError('DEPENDENCY_DUE requires current review and stale authoritative/minimum dependency evidence')
    elif state=='TRANSITION_DUE':
        if review_due or dependency_due or not transition_due:
            raise ValueError('TRANSITION_DUE requires current dependencies and stale management/failure/transition evidence')
    elif state=='GAPS_OPEN':
        if review_due or dependency_due or transition_due or not open_gaps:
    ...
    return {
        ...
        'address_family_profile_ref':scope['address_family_profile_ref'],
...
def validate(index,as_of=None,root=ROOT):
    ...
    for raw in records:
        ...
        if item['bootstrap_id'] in bootstrap_ids:
            raise ValueError('Duplicate active assurance for one bootstrap profile')
    ...
    try:
        ...
        print(json.dumps({
            ...
            **{k:v for k,v in summary.items() if k!='records'},
            ...
            'limits':[
                'Authoritative IPAM and DNS remain separate external authorities.',
                'A reachable resolver/time/repository endpoint is not complete bootstrap readiness.',
                'Temporary dependencies must transition without deleting the only recovery material.',
                'The repository validates exported evidence only and never performs bootstrap mutation.'
            ]
        },indent=2)); return 0
