...
`python scripts/check_storage_data_lifecycle_assurance.py --as-of 2026-09-19T01:50:00Z` validates storage ownership/categorization/access/key/placement lineage, cross-scope denial evidence, measurable service semantics, copy/hold/key-version records and qualified retirement/sanitization procedure evidence. `python scripts/check_storage_data_lifecycle_readiness.py examples/storage_data_lifecycle_readiness_intent.json.example --as-of 2026-09-19T01:50:00Z --expected-status HOLD_NO_CURRENT_STORAGE_LIFECYCLE_ASSURANCE` verifies the active empty storage index remains fail-closed. Tests reject missing cross-scope denial, contention/failure evidence, copy/key lineage or sanitization receipt model while every storage/export/delete/sanitize/apply/activation authority flag remains false.

## Origin-specific shared-service reply assurance

`python scripts/check_service_reply_assurance.py --as-of 2026-09-19T12:30:00Z` validates exact service-binding/origin/endpoint/family scope, entitlement and revocation, forward/reply ownership, alternate-path exclusion, source validation, no-transit proof, failure/recovery behavior and service availability/lifecycle evidence. `python scripts/check_service_reply_readiness.py examples/service_reply_readiness_intent.json.example --as-of 2026-09-19T12:30:00Z --expected-status HOLD_NO_CURRENT_SERVICE_REPLY_ASSURANCE` verifies the active empty reply-path index remains fail-closed. Tests reject missing origin-specific return ownership, alternate-interface/no-transit review, missing-route/reverse-initiation evidence and stale path/binding evidence while all routing/service/apply/activation authority flags remain false.

## Bootstrap service and steady-state transition assurance

`python scripts/check_bootstrap_service_assurance.py --as-of 2026-09-19T14:30:00Z` validates authoritative address/name lifecycle references, selected assignment/DHCP-metadata profile, minimum resolver/time/trust/artifact/telemetry/service-reply dependencies, restricted management/execution, dependency-loss behavior and transfer to steady state. `python scripts/check_bootstrap_service_readiness.py examples/bootstrap_service_readiness_intent.json.example --as-of 2026-09-19T14:30:00Z --expected-status HOLD_NO_CURRENT_BOOTSTRAP_SERVICE_ASSURANCE` verifies the active empty bootstrap index remains fail-closed. Tests reject missing IPAM/DNS, missing artifact/trust dependencies, unrestricted fallback, missing recovery-material preservation, stale dependency/transition evidence and scope mismatch while all IPAM/DNS/DHCP/identity/service/apply/activation authority flags remain false.
