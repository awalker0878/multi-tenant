...
''')
    ...
    def navigation(self):
        ...
        self.category('docs/assurance/README.md','Assurance, audit and acceptance',
          ...
          [('docs/assurance/requirements.md','Complete requirement catalogue'),('docs/assurance/test-specifications.md','Complete inherited test specifications'),('docs/assurance/implementation-audit.md','Implementation coverage and audit boundaries'),('docs/assurance/repository-side-completion-and-residual-audit.md','Repository-side completion and residual audit'),('docs/assurance/registers.md','Original editable registers'),('docs/archive/README.md','Historical audits and source lineage')])
...
''')
    ...
    def code_map(self):
        ...
        text+=['## Platform capability registry','',
          ...
          'Design: '+self.link(p,'docs/engineering/routed-ipv6-qualification.md','Routed IPv6 qualification')+' · '+self.link(p,'docs/implementation/routed-ipv6-lab.md','Routed IPv6 execution')+' · '+self.link(p,'docs/implementation/allocation/ipv6.md','IPv6 assertion allocation'), '',
          'Implementation: '+self.link(p,'sources/capabilities/native_ipv6_assurance_index.json','active native IPv6 assurance index')+' · '+self.link(p,'scripts/check_native_ipv6_assurance.py','assurance validator')+' · '+self.link(p,'scripts/check_native_ipv6_readiness.py','readiness preflight')+' · '+self.link(p,'sources/implementation/routed_ipv6.json','local routed-family evidence')+' · '+self.link(p,'docs/engineering/native-ipv6-address-family-assurance.md','engineering boundary'), '',
          ...
          'Design: '+self.link(p,'docs/implementation/delivery-guide/7-tenant-provisioning-and-controlled-production-activation.md','IK §7 controlled production activation')+' · '+self.link(p,'docs/implementation/provisioning-strategy/4-end-to-end-fixture-provisioning-and-safe-activation.md','PROV §4 safe activation')+' · '+self.link(p,'docs/templates/implementation-mop/7-gate-decision-and-production-activation.md','IT §7 gate decision'), '',
          'Implementation: '+self.link(p,'sources/capabilities/production_activation_assurance_index.json','active production activation assurance index')+' · '+self.link(p,'scripts/check_production_activation_assurance.py','assurance validator')+' · '+self.link(p,'scripts/check_production_activation_readiness.py','readiness preflight')+' · '+self.link(p,'docs/engineering/production-activation-and-initial-readiness-assurance.md','engineering boundary'), '',
          ...
          'Design: '+self.link(p,'docs/architecture/shared-services/3-identity-certificates-keys-and-independent-recovery.md','SVC §3 identity, certificates, keys and independent recovery')+' · '+self.link(p,'docs/implementation/allocation/cry.md','CRY assertion allocation')+' · '+self.link(p,'docs/assurance/requirements.md','IAM/CRY requirements','IAM-001'), '',
          'Implementation: '+self.link(p,'sources/capabilities/identity_crypto_assurance_index.json','active identity/crypto assurance index')+' · '+self.link(p,'scripts/check_identity_crypto_assurance.py','assurance validator')+' · '+self.link(p,'scripts/check_identity_crypto_readiness.py','readiness preflight')+' · '+self.link(p,'docs/engineering/identity-crypto-trust-assurance.md','engineering boundary'), '',
          ...
          'Design: '+self.link(p,'docs/architecture/shared-services/4-storage-copies-and-retained-data-ownership.md','SVC §4 storage and retained-data ownership')+' · '+self.link(p,'docs/implementation/allocation/sto.md','STO assertion allocation')+' · '+self.link(p,'docs/architecture/reference/12-storage-backup-and-data-isolation-architecture.md','Storage/backup architecture'), '',
          'Implementation: '+self.link(p,'sources/capabilities/storage_data_lifecycle_assurance_index.json','active storage lifecycle assurance index')+' · '+self.link(p,'scripts/check_storage_data_lifecycle_assurance.py','assurance validator')+' · '+self.link(p,'scripts/check_storage_data_lifecycle_readiness.py','readiness preflight')+' · '+self.link(p,'sources/capabilities/backup_restore_assurance_index.json','separate backup/restore assurance')+' · '+self.link(p,'docs/engineering/storage-data-lifecycle-assurance.md','engineering boundary'), '',
          ...
          'Design: '+self.link(p,'docs/adr/0035-make-shared-service-replies-select-the-originating-security-context.md','ADR-0035 origin-specific replies')+' · '+self.link(p,'docs/engineering/network-boundaries/2-walk-f14-01-through-the-forward-and-reply-routes.md','NBD §2 forward/reply routes')+' · '+self.link(p,'docs/architecture/shared-services/1-shared-service-placement-and-consumption-boundaries.md','SVC §1 placement and consumption boundaries'), '',
          'Implementation: '+self.link(p,'sources/capabilities/service_reply_assurance_index.json','active service-reply assurance index')+' · '+self.link(p,'scripts/check_service_reply_assurance.py','assurance validator')+' · '+self.link(p,'scripts/check_service_reply_readiness.py','readiness preflight')+' · '+self.link(p,'docs/engineering/origin-specific-service-reply-assurance.md','engineering boundary'), '',
          ...
          'Design: '+self.link(p,'docs/architecture/reference/21-day-0-bootstrap-and-physical-commissioning.md','RA §21 Day-0 bootstrap')+' · '+self.link(p,'docs/implementation/provisioning-strategy/2-day-0-and-steady-state-commissioning-without-circular-dependencies.md','PROV §2 bootstrap without circular dependencies')+' · '+self.link(p,'docs/architecture/shared-services/2-name-time-initialization-and-telemetry-profiles.md','SVC §2 name/time/initialization/telemetry'), '',
          'Implementation: '+self.link(p,'sources/capabilities/bootstrap_service_assurance_index.json','active bootstrap assurance index')+' · '+self.link(p,'scripts/check_bootstrap_service_assurance.py','assurance validator')+' · '+self.link(p,'scripts/check_bootstrap_service_readiness.py','readiness preflight')+' · '+self.link(p,'docs/engineering/authoritative-ipam-allocation-handoff.md','authoritative IPAM handoff')+' · '+self.link(p,'docs/engineering/authoritative-dns-registration-handoff.md','authoritative DNS handoff')+' · '+self.link(p,'docs/engineering/bootstrap-service-readiness-assurance.md','engineering boundary'), '',
          ...
          'Design: '+self.link(p,'docs/implementation/delivery-guide/1-implementation-workplan-and-required-inputs.md','IK §1 implementation workplan')+' · '+self.link(p,'docs/engineering/platform-realizations/7-implementation-tuple-and-decision-package.md','VND §7 implementation tuple')+' · '+self.link(p,'docs/assurance/gap-map/4-open-decision-package-for-implementation.md','GM §4 open decision package'), '',
          'Implementation: '+self.link(p,'sources/capabilities/target_selection_assurance_index.json','active target-selection assurance index')+' · '+self.link(p,'scripts/check_target_selection_assurance.py','assurance validator')+' · '+self.link(p,'scripts/check_target_selection_readiness.py','readiness preflight')+' · '+self.link(p,'docs/engineering/actual-target-selection-assurance.md','engineering boundary'), '',
          ...
          '## Open native work','',self.link(p,'sources/implementation_backlog.csv','The implementation backlog')+' remains the source record for **actual owner decisions and native evidence**. I01–I10 now have repository-side verification, handoff or assurance mechanisms; that does not select a target, populate any active assurance index, contact a native system, issue authorization or turn synthetic evidence into native qualification.']
        self.write(p,'\n'.join(text))
