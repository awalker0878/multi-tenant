...
`python scripts/check_address_family_assurance.py --as-of 2026-09-18T22:45:00Z` validates exact IPv6-only/dual-stack scope, native routing/enforcement/local-protocol evidence, IPv4/IPv6 security equivalence, shared-service dependencies, PMTU/PTB/fragment handling, failure recovery and operating acceptance. `python scripts/check_address_family_readiness.py examples/address_family_readiness_intent.json.example --as-of 2026-09-18T22:45:00Z --expected-status HOLD_NO_CURRENT_ADDRESS_FAMILY_ASSURANCE` verifies the active empty family index remains fail-closed. Tests cover mode/family mismatch, incomplete service dependencies, stale packet/PMTU evidence, stale operations evidence and open gaps while all address/network/apply/activation authority flags remain false.
