...
## Current repository integrity note — completion audit CA-03

The preserved commands above describe the original increment. `python tools/check_release.py` now validates the current clean Git checkout against its own HEAD; it no longer treats the old increment manifest as current. Run before writing local reports. For an exported source tree use an explicitly generated current snapshot manifest with `--manifest`; missing Git metadata is reported as blocked, not a false pass. The old manifest is retained only under `evidence/historical/`. See [current execution instructions](../../TESTING.md).
