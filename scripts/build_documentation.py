...
''')
    ...
    def code_map(self):
        ...
        text+=['## Platform capability registry','',
          ...
          '## Open native work','',
          self.link(p,'docs/implementation/current-status.md','The maintained current-status overlay')+' records repository/local progress while keeping all native qualification and authorization open. '+self.link(p,'sources/implementation_backlog.csv','The inherited implementation backlog')+' is preserved as the historical Increment02–04 source record and is not rewritten to represent later work.']
        self.write(p,'\n'.join(text))
