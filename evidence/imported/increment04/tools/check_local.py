...
def main() -> int:
    ...
    snapshot = {str(p.relative_to(ROOT)): hashlib.sha256(p.read_bytes()).hexdigest()
                for directory in ('tools', 'tests', 'lab', 'terraform')
    ...
    report = {
        ...
        'note': 'Python tests and source inspections do not parse Terraform, execute providers or prove native security behavior.',
    }
