...
def main() -> int:
    ...
    validated = (
        ...
        and all(item["validation"] == "PASSED"
                and item.get("schema_export") == "NOT_RUN_ROOT_BACKEND_BOUNDARY"
                and item.get("matching_module_provider_requirements") is True
                and item.get("lock_sha256") for item in report["roots"]))
